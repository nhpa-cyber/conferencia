...
export default function ConferenteView({
  ...
  const [refugoQty, setRefugoQty] = useState<number>(1);
  ...
  const [refugoPhoto, setRefugoPhoto] = useState<string>('');
  ...
  // Keep activeSession in sync with updates from parent audits (real-time/collaboration sync)
  useEffect(() => {
    if (activeSession) {
  ...
  const handleAddProductToSession = () => {
    ...
    // Filter out any zero quantity item to keep only manually added items with quantity
    const filteredItems = updatedItems.filter(item => {
      if (activeSession.status === 'reconferencia') {
        const physical = item.physicalQty ?? 0;
        const recount = item.rePhysicalQty ?? 0;
        return physical > 0 || recount > 0;
      }
      return (item.physicalQty ?? 0) > 0;
    });

    const updatedSession = { ...activeSession, items: filteredItems };
    setActiveSession(updatedSession);
  ...
  const handleRemoveProduct = (code: string) => {
    if (!activeSession) return;
    const updatedItems = activeSession.items.filter(i => i.productCode !== code);
    const updatedSession = { ...activeSession, items: updatedItems };
    setActiveSession(updatedSession);
    const updatedAudits = audits.map(a => a.id === activeSession.id ? updatedSession : a);
    onSaveAudits(updatedAudits);
    ...
    if (isWebcamSimulated) {
      ...
      if (ctx) {
        ...
        if (webcamTargetRefugo) {
          ...
          ctx.fillText(`MOTIVO: ${refugoReason}`, 130, 190);
          ctx.fillText(`ATIVO: ${activeSession.assets.find(a => a.assetId === refugoAssetId)?.assetName || 'N/A'}`, 130, 210);
        } else if (photoItemTargetType === 'troca_reposicao') {
  ...
  const mapsInCount = importedRoutes.filter(r => r.status !== 'fechado').length;
  ...
  return (
    <div className="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8 py-4 sm:py-8 animate-fade-in" id="conferente_view">
      ...
      {/* Real-time Toast Notifications for closed sheets */}
      {(() => {
        const unreadAlerts = fiscalAlerts.filter(a => !a.read && (a.targetRole === 'conferente' || a.targetRole === 'todos'));
        if (unreadAlerts.length === 0) return null;

        return (
          <div className="mb-6 space-y-2">
            {unreadAlerts.map((alert) => {
              const isRecontagem = alert.status === 'recontagem_solicitada';
              const isFinalizadoOk = alert.status === 'finalizado_ok';
              
              let cardBg = 'bg-red-50 border-red-200 text-red-950';
              let iconColor = 'text-red-600';
              let titleText = 'MAPA BAIXADO NO FISCAL!';
              
              if (isFinalizadoOk) {
                cardBg = 'bg-emerald-50 border-emerald-200 text-emerald-950';
                iconColor = 'text-emerald-600';
              } else if (isRecontagem) {
                cardBg = 'bg-amber-50 border-amber-200 text-amber-950';
                iconColor = 'text-amber-600';
                titleText = 'RECONFERÊNCIA SOLICITADA!';
              }

              return (
                <div 
                  key={alert.id}
                  className={`p-4 rounded-xl shadow-md border flex items-center justify-between text-sm transition-all duration-300 animate-pulse ${cardBg}`}
                >
                  <div className="flex items-center space-x-3">
                    <span className="p-2 bg-white rounded-full block shadow-xs">
                      <Bell className={`h-4 w-4 ${iconColor}`} />
                    </span>
                    <div>
                      <span className="font-bold text-slate-900 font-sans block">{titleText}</span>
                      <span>
                        {isRecontagem ? (
                          <span>{alert.message || `Uma reconferência foi solicitada para o mapa ${alert.routeMap}.`}</span>
                        ) : (
                          <span>
                            O mapa <strong>{alert.routeMap}</strong> (Veículo {alert.plate}) foi dado baixa pelo fiscal: 
                            {isFinalizadoOk ? (
                              <span className="text-emerald-700 font-bold ml-1">✓ Sem Divergências</span>
                            ) : (
                              <span className="text-red-700 font-bold ml-1">⚠️ Com Divergências e Geração de Vales</span>
                            )}
                          </span>
                        )}
                      </span>
                  ...
                  <button
                    ...
                    className="text-xxs font-bold uppercase hover:bg-white px-2 py-1.5 rounded transition shadow-xs cursor-pointer border border-slate-200"
                  >
                    Ciente
                  </button>
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                      ...
                      >
                        <div className="space-y-2">
                          ...
                          <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-slate-600">
                            ...
                            <div>
                              <strong className="text-slate-700">Data:</strong> {audit.arrivalDate}
                            </div>
                          </div>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
              ...
              {importedRoutes && importedRoutes.filter(r => r.status !== 'fechado').length > 0 && (
                <div className="mb-5 bg-slate-50 border border-slate-200 rounded-xl p-3.5 space-y-2">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-[10px] text-slate-500 font-semibold font-mono">
                      {importedRoutes.filter(r => r.status !== 'fechado').length} abertos
                    </span>
                  </div>
                  ...
                  <div className="flex flex-col gap-2">
                    {importedRoutes.filter(r => r.status !== 'fechado').map(route => {
                      ...
                      return (
                        ...
                        >
                          <div 
                            ...
                            title="Clique para selecionar este mapa"
                          >
                            <div className="flex items-center space-x-2">
                              {isPendente && (
                                <input
              ...
              return (
                <div className="bg-red-50 border-l-4 border-red-600 rounded-xl p-5 text-slate-950 shadow-xs space-y-4">
                  ...
                  <div className="bg-white p-4 rounded-lg border border-red-200 mt-3">
                    <h4 className="text-xs font-bold text-slate-900 uppercase tracking-wider mb-3 flex items-center gap-1.5 font-sans">
                      <Sparkles className="h-3.5 w-3.5 text-red-600 animate-spin-slow" />
                      Registro de Cobertura da Blitz de Refugo
                    </h4>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-[10px] font-bold text-slate-700 uppercase mb-1 font-mono">
                          Qtd de Caixas Conferidas (Rebatidas)
                        </label>
                        <input
                          type="number"
                          min="0"
                          value={activeSession.blitzBoxesChecked ?? ''}
                          onChange={(e) => {
                            const val = e.target.value === '' ? 0 : parseInt(e.target.value, 10);
                            handleUpdateBlitzData(val, activeSession.blitzAvariasFound ?? 0);
                          }}
                          placeholder="Ex: 120"
                          className="w-full bg-slate-50 border border-slate-300 rounded-lg px-3 py-2 text-xs text-slate-900 focus:outline-none focus:ring-2 focus:ring-red-500 font-bold"
                        />
                      </div>
                      <div>
                        <label className="block text-[10px] font-bold text-slate-700 uppercase mb-1 font-mono">
                          Qtd de Avarias Encontradas na Blitz
                        </label>
                        <input
                          type="number"
                          min="0"
                          value={activeSession.blitzAvariasFound ?? ''}
                          onChange={(e) => {
                            const val = e.target.value === '' ? 0 : parseInt(e.target.value, 10);
                            handleUpdateBlitzData(activeSession.blitzBoxesChecked ?? 0, val);
                          }}
                          placeholder="Ex: 5"
                          className="w-full bg-slate-50 border border-slate-300 rounded-lg px-3 py-2 text-xs text-slate-900 focus:outline-none focus:ring-2 focus:ring-red-500 font-bold"
                        />
                    ...
                    <p className="text-[9px] text-slate-500 mt-2 font-sans italic">
                      * Estes dados serão consolidados no painel da gestão para aferição do Índice de Refugo real da operação.
                    </p>
                  </div>
                </div>
              );
          ...
          {activeSession.isSuspended ? (
            ...
            <div className="lg:col-span-5 space-y-6">
              ...
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 space-y-4" id="section_refugos_ag">
                ...
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-150 grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-3">
                    <div>
                      <label className="block text-[10px] font-bold text-slate-600 uppercase mb-1 font-sans">1. Selecione o Ativo com Avaria</label>
                      <select
                        id="refugo_asset_select"
                        value={refugoAssetId}
                        onChange={(e) => setRefugoAssetId(e.target.value)}
                        className="w-full text-xs p-2 bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-red-500 font-sans"
                      >
                    ...
                    <div>
                      <label className="block text-[10px] font-bold text-slate-600 uppercase mb-1 font-sans">2. Motivo da Avaria / Refugo</label>
                      <select
                        id="refugo_reason_select"
                        value={refugoReason}
                        onChange={(e) => setRefugoReason(e.target.value as any)}
                        className="w-full text-xs p-2 bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-red-500 font-sans"
                      >
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-[10px] font-bold text-slate-600 uppercase mb-1 font-sans">3. Qtd Refugada</label>
                        <input
                          type="number"
                          min="1"
                          id="refugo_qty_input"
                          value={refugoQty}
                          onChange={(e) => setRefugoQty(Math.max(1, Number(e.target.value) || 1))}
                          className="w-full text-xs p-2 bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-red-500 font-mono text-center font-bold"
                        />
                      </div>
                      <div className="flex flex-col justify-end">
                        <button
                          ...
                          onClick={() => {
                            ...
                            const selectedAsset = activeSession.assets.find(a => a.assetId === refugoAssetId);
                            if (!selectedAsset) return;

                            const refugoId = 'ref_' + Date.now();
                            const newRefugo = {
                              id: refugoId,
                              ...
                              qty: refugoQty,
                              reason: refugoReason,
                              photoUrl: refugoPhoto || undefined
                            };
                            ...
                            if (refugoPhoto) {
                              savePhotoRecordToDb(
                                refugoPhoto,
                                refugoId,
                                `Refugo de Ativo: ${selectedAsset.assetName} (${refugoReason})`,
                                'refugo'
                              );
                            }

                            const updatedRefugos = [...(activeSession.refugos || []), newRefugo];
                          ...
                          className="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-3 rounded-lg text-xs cursor-pointer transition shadow-xs text-center font-sans uppercase"
                        >
                          Adicionar
                        </button>
                      </div>
                  ...
                  {/* Refugo Camera integration */}
                  <div className="flex flex-col justify-between border-l border-slate-200 pl-0 md:pl-4 space-y-3">
                    <div className="space-y-2">
                      <span className="block text-[10px] font-bold text-slate-600 uppercase font-sans">4. Evidência Fotográfica do Refugo</span>
                      <p className="text-[10px] text-slate-500">Tire uma foto ou anexe uma imagem que comprove o motivo do refugo.</p>
                      
                      {refugoPhoto ? (
                        <div className="relative w-full h-32 rounded-lg overflow-hidden border border-slate-300 shadow-sm bg-slate-900">
                          <img src={refugoPhoto} alt="Refugo" className="w-full h-full object-contain" referrerPolicy="no-referrer" />
                          <button
                            type="button"
                            onClick={() => setRefugoPhoto('')}
                            className="absolute top-1.5 right-1.5 bg-red-600 hover:bg-red-700 text-white p-1 rounded-full shadow-md transition cursor-pointer"
                            title="Remover foto"
                          >
                            <Trash2 className="h-3.5 w-3.5" />
                          </button>
                        ...
                        <div className="border-2 border-dashed border-slate-300 rounded-lg p-4 text-center bg-slate-100/50 flex flex-col items-center justify-center space-y-1 h-32">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <button
                        ...
                        className="bg-slate-800 hover:bg-slate-900 text-white font-bold py-2 px-2 rounded-lg text-[10px] flex items-center justify-center space-x-1 cursor-pointer transition shadow-2xs font-sans"
                      >
                        <Camera className="h-3 w-3" />
                        <span>Tirar Foto</span>
                      </button>

                      <label className="bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold py-2 px-2 rounded-lg text-[10px] flex items-center justify-center space-x-1 cursor-pointer border border-slate-300 transition shadow-2xs text-center font-sans">
                        <span>Anexar Foto</span>
                        <input
                          type="file"
                          accept="image/*"
                              ...
                              reader.onload = (event) => {
                                ...
                                img.onload = () => {
                                  ...
                                  if (ctx) {
                                    ...
                                    ctx.font = '8px sans-serif';
                                    ctx.fillText(`Mapa: ${activeSession.routeMap} • Motivo: ${refugoReason} • Data: ${new Date().toLocaleString()}`, 10, 348);
